...
name: {
  ...
  methods: {
    async initialize() {
      ...
      if (responses) {
        this.level = responses[1].data.level;
this.area = responses[2].data.area;
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await locationApi.put(`/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await locationApi.post(null, this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await locationApi.delete(`/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await locationApi.get(null, {
