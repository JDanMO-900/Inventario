  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await backendApi.put(`/typeAction/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await backendApi.post('/typeAction', this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await backendApi.delete(`/typeAction/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await backendApi.get('/typeAction', {
