      ...
      <v-card>
        ...
        <!-- name -->
            <v-col cols="12" sm="12" md="12">
                <base-input
                label="Ingrese nombre de la dependencia"
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await backendApi.put(`/dependency/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await backendApi.post('/dependency', this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await backendApi.delete(`/dependency/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await backendApi.get('/dependency', {
