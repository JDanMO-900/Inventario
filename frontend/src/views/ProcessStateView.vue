...
export default {
  ...
  data() {
    return {
      ...
      records: [],
      editedIndex: -1,
      title: "Listado de nombre de procesos",
      total: 0,
      options: {},
      editedItem: {
        		name: "",
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await backendApi.put(`/processState/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await backendApi.post('/processState', this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await backendApi.delete(`/processState/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await backendApi.get('/processState', {
