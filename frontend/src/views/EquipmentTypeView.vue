  ...
  data() {
    return {
      ...
      headers: [
		...
		{ title: "Tipo de equipo", key: "name" },
        { title: "ACCIONES", key: "actions", sortable: false },
      ],
      records: [],
      editedIndex: -1,
      title: "Listado de tipo de equipos",
      total: 0,
      options: {},
      editedItem: {
        		name: "",
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await backendApi.put(`/equipmentType/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await backendApi.post('/equipmentType', this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await backendApi.delete(`/equipmentType/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await backendApi.get('/equipmentType', {
