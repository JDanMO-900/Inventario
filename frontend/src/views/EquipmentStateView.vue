          ...
          <v-container>
            ...
            <v-row class="pt-3">
              ...
              <v-col cols="12" sm="12" md="12">
                <base-input label="Ingresa el estado del equipo" v-model="v$.editedItem.name.$model" :rules="v$.editedItem.name" />
              </v-col>
              <!-- name -->
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await backendApi.put(`/equipmentState/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await backendApi.post('/equipmentState', this.editedItem);
    ...
    async deleteItemConfirm() {
      try {
        const { data } = await backendApi.delete(`/equipmentState/${this.editedItem.id}`, {
          params: {
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await backendApi.get('/equipmentState', {
