      ...
      <v-card>
        ...
        <!-- name -->
            <v-col cols="12" sm="12" md="12">
                <base-input
                label="Ingrese descripción técnica"
...
import { minLength, required, email } from "@/lang/i18n";
  ...
  data() {
    return {
      ...
      headers: [
		...
		{ title: "Descripción técnica", key: "name" },
        { title: "Acciones", key: "actions", sortable: false },
      ],
      records: [],
      editedIndex: -1,
      title: "Descripciones técnicas ",
      total: 0,
      options: {},
      editedItem: {
        		name: "",
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        ...
        try {
          const { data } = await backendApi.put(`/technicalDescription/${edited.id}`, edited);
      ...
      //Creating record
      try {
        const { data } = await backendApi.post('/technicalDescription', this.editedItem);
    ...
    getDataFromApi(options) {
      ...
      this.debounce = setTimeout(async () => {
        try {
          const { data } = await backendApi.get('/technicalDescription', {
